...
<div class="container-xxl flex-grow-1 container-p-y">
    <form id="mydata" action="{{ route('update.fourday.package', $package->id) }}" method="POST">
        @csrf
        @method('PUT')
                        ...
                        <div class="card-body">
                            <div class="row mb-6">
                                <div class="col">
                                    ...
                                    <input type="text" class="form-control" id="name_package"
                                        placeholder="Name Package" value="{{ $package->name_package }}"
                                    ...
                                    <select required id="city_district" name="cityOrDistrict_id"
                                        ...
                                        @foreach ($regencies as $regency)
                                            <option value="{{ $regency->id }}"
                                                {{ $package->regency_id == $regency->id ? 'selected' : '' }}>
                                                {{ $regency->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="row mb-6">
                                    ...
                                    <select required id="name_agen" name="NameAgen" class="select2 form-select"
                                        ...
                                        @foreach ($agens as $agen)
                                            <option value="{{ $agen->id }}"
                                                {{ $package->agen_id == $agen->id ? 'selected' : '' }}>
                                ...
                                <div class="col-12">
                                    ...
                                    <textarea rows="3" class="mb-3 d-none" name="information" id="quill-editor-area">{{ $package->information }}</textarea>
                                </div>
                                <!-- /Full Editor -->
                            </div>
                        </div>
                                ...
                                <select id="facility" name="facilities[]" class="select2 form-select" multiple>
                                    @foreach ($facilities as $facility)
                                        <option value="{{ $facility->id }}"
                                            {{ in_array($facility->id, $selectedFacilities) ? 'selected' : '' }}>
