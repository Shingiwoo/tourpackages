                ...
                @if ($package->type == 'custom')
                    <div class="card p-4">
                        <div class="text-center mb-6">
                            ...
                            <p class="address-subtitle">Trip Detail for <span
                                    class="text-primary">{{ $package->duration }}</span> day(s) <span
                                    class="text-primary">{{ $package->night }}</span> night(s)</p>
                        </div>
                        <div class="row mb-4">
                                            ...
                                            <div class="demo-inline-spacing mt-4">
                                                <ol class="list-group list-group-numbered">
                                                    @forelse ($package->destinations as $desti)
                                                        <li
                                                            class="list-group-item list-group-item-action waves-effect waves-light">
                                                            {{ $desti->name }}</li>
                                                    @empty
                                                        <li
                                                            class="list-group-item list-group-item-action waves-effect waves-light">
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <td class="text-end perperso-cost">Rp
                                            {{ number_format($package->costPerPerson, 0, ',', '.') }}</td>
                                    </tr>
                                    <tr>
                            ...
                            <div class="row mt-4">
                                ...
                                <ul>
                                    <li class="mb-2">Biaya Anak-anak: <strong class="child-cost">Rp
                                            {{ number_format($package->childCost, 0, ',', '.') }}</strong>
                                    ...
                                    <li>Biaya Tambahan WNA: <strong class="additional-cost-wna">Rp
                                            {{ number_format($package->additionalCostWna, 0, ',', '.') }}</strong><br><span
                        ...
                        <div class="accordion-item card text-warning">
                            <h2 class="accordion-header d-flex align-items-center">
                                <button type="button" class="accordion-button collapsed" data-bs-toggle="collapse"
                                    ...
                                    <h5 class="text-warning text-uppercase"><i class="me-2 mb-1 ti ti-receipt-2"></i> Price
                                        List
                                    </h5>
                                </button>
                            </h2>
                                        ...
                                        <div class="table-responsive text-nowrap">
                                            @if ($package->type == 'oneday')
                                                <table class="table table-bordered">
                                                    <thead>
                                                        <tr>
                                                            <th class="align-content-center text-center text-warning">
                                                                Vehicle</th>
                                                            <th class="align-content-center text-center text-warning">User
                                                            </th>
                                                            <th class="align-content-center text-center text-warning">Price
                                                            </th>
                                                            <th class="align-content-center text-center text-warning">Wna
                                                                Cost</th>
                                                        </tr>
                                                    </thead>
                                                    @php
                                                            ...
                                                            @foreach ($prices as $price)
                                                                <tr>
                                                                    <td class="align-content-center text-center text-info">
                                                                        {{ $price['vehicle'] }}</td>
                                                                    <td
                                                                        class="align-content-center text-center text-warning">
                                                                        {{ $price['user'] }}</td>
                                                                    <td class="align-content-center text-center text-info">
                                                                        Rp
                                                                    ...
                                                                    <td class="align-content-center text-center text-info">
                                                                        Rp
                                                                        {{ number_format($price['wnaCost'], 0, ',', '.') }}
                                                                        /orang</td>
                                                                </tr>
                                                            @endforeach
                                                        @else
                                                            <tr>
                                                                <td colspan="3" class="text-center">No price data
                                                                    available</td>
                                                            </tr>
                                                        @endif
                                                    </tbody>
                                            ...
                                            @else
                                                <table class="table table-bordered">
                                                    ...
                                                    <tbody>
                                                        @if (is_array($prices) && count($prices) > 0)
                                                            @foreach ($prices as $priceRow)
                                                                <tr>
                                                                    ...
                                                                    <td class="align-content-center text-center">
                                                                        {{ number_format($priceRow['wnaCost'], 0, ',', '.') }}
                                                                        /org</td>
                                                                    @foreach ($accommodationTypes as $type)
                                                                        <td class="align-content-center text-center">
                                                                            {{ number_format($priceRow[$type] ?? 0, 0, ',', '.') }}
                                                                            /org</td>
                                                                    @endforeach
                                                                </tr>
                                                            @endforeach
                                                        @else
                                                            <tr>
                                                                <td colspan="{{ 3 + count($accommodationTypes) }}"
                                                                    class="text-center">
                ...
                <div class="card mb-6">
                    <div class="card-body">
                        ...
                        @if (Auth::user()->can('booking.add'))
                            <a href="javascript:void(0)" class="btn btn-label-success w-100 mb-4 me-2"
                                data-bs-toggle="modal" data-id="{{ $package->id }}" data-bs-target="#bookingModal"> <i
